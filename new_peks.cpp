...
std::pair<element_t*, char*>* peksOpt::PEKS(element_t* g, element_t* h, pairing_t* pairing, element_t* H1_W, int bitswanted)
{
	char *H2_t = B;
	element_t r, hR, t;
	...
	auto out= std::make_pair(&peks, B);
	return &out;
}

element_t* peksOpt::Trapdoor(pairing_t *pairing, element_t *alpha, element_t *H1_W)
{
...
int peksOpt::Test(element_t* g, element_t* h, std::pair<element_t*, char*>* peks, element_t* Tw, pairing_t pairing)
{
	/* PEKS for W2S */
...
//#endif
	...
	pairing_apply(temp, *Tw, *peks->first, pairing);
